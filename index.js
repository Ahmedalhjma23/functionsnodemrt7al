    ...
    // إعداد اتصال Mongoose بدون الخيارات المهجورة
    ...
    if (cachedData) {
        ...
        try {
        // في حالة عدم وجود cachedData، نحاول جلب البيانات من قاعدة البيانات
        const latestData = await DataModel.findOne().sort({ fetchedAt: -1 }).lean();
        if (latestData) {
            res.json(latestData);
        } else {
